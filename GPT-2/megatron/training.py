...
def forward_step(
    data_iterator, model, neox_args, timers, return_logits=False, return_acc=False, is_train=False
):
    """Forward step."""
    ...
    if neox_args.use_tempnet and is_train:
        ...
        #with open(f"outputs_taus_{neox_args.seed}.pkl", 'wb') as f:
        #    pickle.dump({"logits": outputs.cpu().detach().numpy(), "labels": labels.cpu().detach().numpy(), "taus": taus.cpu().detach().numpy()}, f)

        #print("taus:", taus.min(), taus.mean(), taus.max())
        #assert 0

    else:
        forward_temp = 1.0
        if is_train:
            forward_temp = neox_args.temp

        loss = cross_entropy(
            outputs, (labels, loss_mask), _fp16=neox_args.fp16_lm_cross_entropy, temp=forward_temp
        )

    if return_logits:
        return loss, outputs

    if return_acc:  # only True for evaluation
        bsz, seq_len, vocab_size = outputs.shape
        acc1, acc5 = accuracy(outputs.reshape(bsz*seq_len, vocab_size), labels.reshape(-1))
        return loss, (acc1, acc5)

    if neox_args.use_tempnet and is_train:
        return loss, taus.mean(), tau_grad
    else:
        return loss
...
def train_step(neox_args, timers, data_iterator, model, optimizer, lr_scheduler):
    ...
    if neox_args.is_pipe_parallel:
        ...
        # set the learning rate of temperature network again
        if neox_args.use_tempnet and len(optimizer.param_groups) > 2:
            optimizer.param_groups[2]["lr"] = neox_args.tempnet_lr
        ...
        for _ in range(neox_args.gradient_accumulation_steps):
            ...
            if neox_args.use_tempnet:
                loss, taus, tau_grad = ret_vals
                taus_list.append(taus)
                tau_grad_list.append(tau_grad)
            else:
                loss = ret_vals
        ...
        if neox_args.use_tempnet:
            ...
            reduced_loss["tau_grad"] = reduce_losses(tau_grad_list).mean()
...
def evaluate(
        ...
        while iteration < neox_args.eval_iters:
            ...
            for _ in range(
                ...
                if neox_args.is_pipe_parallel:
                    ...
                    loss, (acc1, acc5) = forward_step_fn(
                        model=model,
                        data_iterator=data_iterator,
                        neox_args=neox_args,
                        timers=timers,
    ...
    if not neox_args.is_pipe_parallel:
        eval_results["acc_1"] = reduce_losses(acc1_list).mean().item()
        eval_results["acc_5"] = reduce_losses(acc5_list).mean().item()
