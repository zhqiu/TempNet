...
from megatron.model.tempnet import TempNet
...
def cross_entropy(output, labels, _fp16=False, temp=1.0):
    """From pretrain_gpt2:forward_step()"""
    labels, loss_mask = labels[0], labels[1]

    bsz, seq_len, vocab_size = output.shape
    losses = temp * nn.functional.cross_entropy(output.reshape(bsz*seq_len,vocab_size) / temp, labels.reshape(-1))
...
class GPT2ModelPipe(PipelineModule, torch.nn.Module):
    ...
    def to_sequential(self):
        ...
        if self.neox_args.use_tempnet:
            model.tempnet = TempNet(rho=self.neox_args.tempnet_rho)
