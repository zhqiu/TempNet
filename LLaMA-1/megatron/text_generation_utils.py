...
def forward_model(model, model_inputs, neox_args) -> torch.Tensor:
    """
    ...
    """
    is_pipe_parallel = neox_args.is_pipe_parallel
    ...
    if not is_pipe_parallel:
        tokens, position_ids, attention_mask = model_inputs
        return model((tokens, position_ids, attention_mask), neox_args=neox_args)
    else:
    ...
    with torch.no_grad():
        ...
        while token_index_to_generate <= last_token_index_to_generate:
            if recompute:  # recompute all tokens
                ...
                #logits = forward_model(model, model_inputs, neox_args.is_pipe_parallel)
                logits = forward_model(model, model_inputs, neox_args)
