...
from megatron.model.tempnet import tau_cross_entropy
...
def forward_step(
    ...
    """
        hack in and check taus for input samples
    """
    # alpaca sft data
    #folder = '/data1/qiuzh/llama_exp/alpaca_data'
    #input_name = 'alpaca_texts_and_tokens.pkl'
    #output_name = 'texts_and_taus.pkl'

    # subjqa data
    folder = '/data1/qiuzh/llama_exp/subjqa_data'
    input_name = 'subjqa_texts_and_tokens.pkl'
    output_name = 'subjqa_texts_and_taus.pkl'

    with open(os.path.join(folder, input_name), 'rb') as f:
        data = pickle.load(f)
    ...
    for entry in tqdm(data):
        tokens = entry[1]
        tokens_len = min(len(tokens), 2048)

        local_tokens = torch.tensor(tokens[:tokens_len]).to(labels.device).unsqueeze(0)
        local_position_ids = torch.arange(tokens_len).to(labels.device).unsqueeze(0)
        local_attention_mask = torch.triu(torch.ones(tokens_len, tokens_len), diagonal=1).bool()[None,None,...].to(labels.device)

        outputs = model((local_tokens, local_position_ids, local_attention_mask), neox_args=neox_args)
        taus = model.module.tempnet(outputs.reshape(tokens_len, 32000).detach())

        texts_and_taus.append([entry[0], taus.mean().item(), entry[2], entry[3], entry[4], entry[5]])

    with open(os.path.join(folder, output_name), 'wb') as f:
        pickle.dump(texts_and_taus, f)
    ...
    if neox_args.use_tempnet and is_train:
        bsz, seq_len, feat_dim = outputs.shape
        taus = model.module.tempnet(outputs.reshape(bsz*seq_len, feat_dim).detach())
        loss, tau_grad = tau_cross_entropy(outputs, (labels, loss_mask), taus, rho=neox_args.tempnet_rho)
    ...
    if return_logits:
        ...
        with torch.no_grad():
            taus = model.module.tempnet(outputs.reshape(bsz*seq_len, feat_dim).detach())
        return loss, outputs / taus.reshape(bsz, seq_len, 1)
        """
        return loss, outputs

    if neox_args.use_tempnet and is_train:
        return loss, taus.mean()
...
def get_model(neox_args, use_cache=False):
    ...
    # freeze everything but the tempnet
    if neox_args.use_tempnet:
        print("freeze everything but the tempnet")
        for name, param in model.named_parameters():
            if not "tempnet" in name:
                param.requires_grad = False
...
def train_step(neox_args, timers, data_iterator, model, optimizer, lr_scheduler):
    ...
    if neox_args.is_pipe_parallel:
        ...
        if neox_args.use_tempnet:
            reduced_loss["tau"] = reduce_losses(taus_list).mean()
