...
#    assert len(list(base_path.glob("mp_rank_*"))) == 1, "Unshard your model with checkpoint_util.py first!"
    ...
    args = loaded['args']
    temp_dict = {}
    for k, v in loaded['model']['tempnet_model'].items():
        if "scaler" in k : v = v.view(1)
        temp_dict["tempnet." + k] = v.float()
    #print(temp_dict)
    safetensors.torch.save_file(temp_dict, os.path.join(model_path, "tempnet.safetensors"), metadata={"format" : "pt"})
    torch.save(temp_dict, os.path.join(model_path, "tempnet.bin"))
    return
